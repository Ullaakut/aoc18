...
// including its size as a third parameter after the top-left coordinate: a 9x9 square with a top-left corner
...
func TestSolveExercise(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(fmt.Sprintf("%d/%d/%d", test.serialNumber, test.expectedX, test.expectedY), func(t *testing.T) {
			x, y, size := solveExercise(test.serialNumber)

			if x != test.expectedX || y != test.expectedY || size != test.expectedSize {
				t.Errorf("expected coordinates to be %d,%d, got %d,%d", test.expectedX, test.expectedY, x, y)
				t.Errorf("expected size to be %d, got %d", test.expectedSize, size)
			}
		})
	}
}
