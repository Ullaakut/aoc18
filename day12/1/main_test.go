...
//  Your puzzle input contains a list of pots from 0 to the right and whether they do (#) or do not (.) currently contain a plant,
//  the initial state. (No other pots currently contain plants.) For example, an initial state of #..##.... indicates that pots
...
//  spread to nearby pots. Based on the notes, for each generation of plants, a given pot has or does not have a plant based
//  on whether that pot (and the two pots on either side of it) had a plant in the last generation. These are written as LLCRR => N,
//  where L are pots to the left, C is the current pot being considered, R are the pots to the right, and N is whether the current pot
//  will have a plant in the next generation. For example:

// A note like ..#.. => . means that a pot that contains a plant but with no plants within two pots of it will not have a plant in
...
// negative-numbered pots extend to the left, and positive pots extend toward the right. Remember, the initial state begins at pot 0, which is not
// the leftmost pot used in this example.

// After one generation, only seven plants remain. The one in pot 0 matched the rule looking for ..#.., the one in pot 4 matched the rule looking
// for .#.#., pot 9 matched .##.., and so on.

// In this example, after 20 generations, the pots shown as # contain plants, the furthest left of which is pot -2, and the furthest right of which
//  is pot 34. Adding up all the numbers of plant-containing pots after the 20th generation produces 325.
...
func TestSolveExercise(t *testing.T) {
	t.Run("known output", func(t *testing.T) {
		expectedSum := 325
		sum := solveExercise(testInputFilePath)
