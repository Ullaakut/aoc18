...
// If a referenced child node does not exist, that reference is skipped. A child node
...
func TestSolveExercise(t *testing.T) {
	t.Run("known output", func(t *testing.T) {
		expectedRootNodeValue := 66
		RootNodeValue := solveExercise(testInputFilePath)

		if RootNodeValue != expectedRootNodeValue {
			t.Errorf("expected root node value to be %d, got %d", expectedRootNodeValue, RootNodeValue)
		}
	})
}
